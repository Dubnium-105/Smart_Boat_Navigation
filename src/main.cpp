...
// --- 新增 ---
TaskHandle_t imageProcessingTaskHandle = NULL;
// --- 结束新增 ---
...
void imageProcessingTask(void *pvParameters) {
  Serial.printf("图像处理任务已启动，运行在核心 %d\\n", xPortGetCoreID());

  TickType_t lastWakeTime = xTaskGetTickCount();
  const TickType_t taskFrequency = pdMS_TO_TICKS(300); // 每 300ms 运行一次
  ...
  while (true) {
    ...
    if (fb && fb->format == PIXFORMAT_GRAYSCALE) {
      ...
      // 输出结果 (减少输出频率或移除 print_ascii_frame 以优化)
      Serial.printf("[最亮点@Core%d] x=%d, y=%d (耗时 %lu ms)\\n", xPortGetCoreID(), bx, by, end_time - start_time);
      ...
      esp_camera_fb_return(fb); // 处理完后立即返回帧缓冲
    } else if (fb) {
      // 如果获取到帧但格式不对，也要返回
      esp_camera_fb_return(fb);
      Serial.println("获取到帧，但格式不是灰度图");
    } else {
...
void setup() {
  ...
  // 初始化性能监控变量 (这些变量主要由 stream.cpp 使用)
  // lastFPSCalculationTime = esp_timer_get_time(); // esp_timer_get_time() is deprecated
  lastFPSCalculationTime = millis(); // Use millis() or esp_timer_get_time_us() / 1000
  frameCount = 0;

  // 启动新的简化视频流服务器 (假设它在内部创建自己的任务)
  startSimpleCameraStream();

  // --- 新增: 创建并启动图像处理任务 ---
  // 将任务固定到核心 1 (核心 0 通常用于 WiFi/BT)
  // 增加堆栈大小以防万一 (图像处理可能需要较多栈空间)
  xTaskCreatePinnedToCore(
      imageProcessingTask,      /* 任务函数 */
      "ImageProc",              /* 任务名称 */
      4096,                     /* 堆栈大小 (字节) */
      NULL,                     /* 任务参数 */
      1,                        /* 任务优先级 (1较低) */
      &imageProcessingTaskHandle, /* 任务句柄 */
      1                         /* 固定到核心 1 */
  );
  if (imageProcessingTaskHandle == NULL) {
      Serial.println("创建图像处理任务失败!");
  } else {
      Serial.println("图像处理任务已创建并固定到核心 1");
  }
  // --- 结束新增 ---
...
void loop() {
  ...
  if (currentTime - lastDiagTime >= 10000) {  // 每10秒
    // 打印基本状态信息
    Serial.printf("[主循环@Core%d] 运行中, FPS: %.2f, Heap: %d, PSRAM: %d\\n",
                  xPortGetCoreID(),
                  currentFPS, // currentFPS 由 stream.cpp 更新
                  esp_get_free_heap_size(),
                  psramFound() ? ESP.getFreePsram() : 0);

    // 可以选择性地调用诊断函数，但注意它会打印很多信息
    // diagnoseCameraPerformance();

    lastDiagTime = currentTime;
  }

  // 短暂延时，让出 CPU 给其他任务，特别是空闲任务
  // vTaskDelay(pdMS_TO_TICKS(10)); // 使用 FreeRTOS 的延时更好
  delay(10); // 或者继续使用 delay，对于简单场景影响不大
}
